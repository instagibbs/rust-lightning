...
This indicates a bug inside LDK. Please report this error at https://github.com/lightningdevkit/rust-lightning/issues/new";
	...
	fn handle_channel_resumption(&self, pending_msg_events: &mut Vec<MessageSendEvent>,
		...
		mut funding_tx_signed: Option<FundingTxSigned>,
		...
		let logger = WithChannelContext::from(&self.logger, &channel.context, None);
		log_trace!(logger, "Handling channel resumption with {} RAA, {} commitment update, {} pending forwards, {} pending update_add_htlcs, {}broadcasting funding, {} channel ready, {} announcement, {} tx_signatures, {} teleport_complete_ack, {} tx_abort, {} splice_locked",
			...
			match peer_state.channel_by_id.entry(msg.channel_id.clone()) {
				hash_map::Entry::Occupied(mut chan_entry) => {
					if let Some(chan) = chan_entry.get_mut().as_funded_mut() {
						logger = WithChannelContext::from(&self.logger, &chan.context, None);
						let res =
							chan.closing_complete(&msg, &peer_state.latest_features, &&logger);
						...
						return try_channel_entry!(
							...
							peer_state,
							Err(ChannelError::close(
								"Got a closing_complete message for an unfunded channel!".into()
							)),
							chan_entry
						);
					}
				...
				hash_map::Entry::Vacant(_) => {
					...
					peer_state.pending_msg_events.push(MessageSendEvent::SendChannelReestablish {
						...
						msg: msgs::ChannelReestablish {
							...
							next_local_nonces: vec![(Txid::all_zeros(), PublicNonce::from_byte_array(&PUBLIC_NONCE).unwrap())],
						},
					});
