...
pub fn remove_first_msg_event_to_node(
	...
	let ev_index = msg_events.iter().position(|e| match e {
		...
		MessageSendEvent::SendTeleportInit { node_id, .. } => node_id == msg_node_id,
		MessageSendEvent::SendTeleportAck { node_id, .. } => node_id == msg_node_id,
		MessageSendEvent::SendTeleportAbort { node_id, .. } => node_id == msg_node_id,
		MessageSendEvent::SendTeleportComplete { node_id, .. } => node_id == msg_node_id,
		MessageSendEvent::SendTeleportCompleteAck { node_id, .. } => node_id == msg_node_id,
