		...
		macro_rules! get_htlc_in_commitment {
			($htlc: expr, $offered: expr) => {
				HTLCOutputInCommitment {
					...
					ark_htlc_success_csv_delta: funding.channel_transaction_parameters.ark_htlc_success_csv_delta,
				}
			}
		}
	...
	SP::EcdsaSigner: EcdsaChannelSigner,
{
	...
	fn teleport_initial_commitment_signed<F: FeeEstimator, L: Logger>(
		...
		let monitor_update = ChannelMonitorUpdate {
			...
			updates: vec![ChannelMonitorUpdateStep::RenegotiatedFunding {
				...
				// Ark bridge: a teleport re-points funding without an interactive-tx splice
				// contribution, so there is nothing to record here.
	...
	pub fn funding_contributed<F: FeeEstimator, L: Logger>(
		...
		match self.quiescent_action.as_ref() {
			...
			// Ark bridge: a teleport is a separate quiescent action that never goes through the
			// splice funding-contribution flow; treat a contribution arriving while one is pending
			// as a no-op rather than corrupting the teleport.
	...
	pub fn propose_quiescence<L: Logger>(
		...
		if !self.context.is_usable() {
			...
			return Err(match action {
				...
				// Ark bridge: a teleport has no in-flight funding contribution to discard.
